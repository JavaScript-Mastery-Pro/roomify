...
import { ArrowRight, Clock, ArrowUpRight, Box, Shield, Layers, Zap } from 'lucide-react';
import { Button } from './ui/Button';
import Upload from './Upload';
import { DesignHistoryItem } from '../App';
...
const Landing: React.FC<LandingProps> = ({ onStart, history, onSelectHistory }) => {
    ...
    <div className="min-h-screen bg-background relative overflow-x-hidden text-foreground">
        ...
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
           <div className="flex items-center space-x-8">
              ...
              <div className="hidden md:flex items-center space-x-6">
                 <a href="#" className="text-sm font-medium text-zinc-600 hover:text-black transition-colors">Product</a>
                 <a href="#" className="text-sm font-medium text-zinc-600 hover:text-black transition-colors">Pricing</a>
                 <a href="#" className="text-sm font-medium text-zinc-600 hover:text-black transition-colors">Community</a>
                 <a href="#" className="text-sm font-medium text-zinc-600 hover:text-black transition-colors">Enterprise</a>
              </div>
           </div>
           
           <div className="flex items-center space-x-4">
              <button className="text-xs font-bold uppercase tracking-wide text-zinc-900 hover:text-primary transition-colors">Log In</button>
              <Button size="sm" onClick={scrollToUpload} className="rounded-md">Get Started</Button>
           </div>
        </div>
      </nav>
      ...
      <section className="pt-32 pb-20 px-6 max-w-7xl mx-auto flex flex-col items-center text-center">
         ...
         <div className="mb-8 inline-flex items-center px-3 py-1 rounded-md bg-white border border-zinc-200 shadow-sm">
            ...
            <span className="text-[10px] font-bold uppercase tracking-widest text-zinc-600">Introducing Roomify 2.0</span>
         </div>
         ...
         <div className="flex flex-col sm:flex-row gap-4 justify-center mb-16">
            <Button size="lg" onClick={scrollToUpload} className="px-8 bg-[#F97316] hover:bg-[#ea580c] text-white border-none rounded-md shadow-md">
               Start Building <ArrowRight className="ml-2 w-4 h-4" />
            </Button>
            <Button variant="outline" size="lg" className="px-8 bg-white rounded-md hover:bg-zinc-50">
               Watch Demo
            </Button>
         </div>

         {/* Hero Visual / Upload Box */}
         <div ref={uploadRef} className="w-full max-w-5xl mx-auto relative rounded-3xl overflow-hidden bg-gradient-to-b from-blue-100/50 to-white/50 border border-blue-100 p-6 md:p-12 shadow-sm min-h-[400px] flex items-center justify-center">
             ...
             <div className="relative w-full max-w-xl bg-white rounded-2xl shadow-xl overflow-hidden border border-zinc-100 p-8 z-10 transition-transform hover:scale-[1.01] duration-500">
                 <div className="text-center mb-6">
                    ...
                    <h3 className="text-xl font-serif font-bold text-black">Upload your floor plan</h3>
                    <p className="text-zinc-500 text-sm mt-1">Supports JPG, PNG formats up to 10MB</p>
                 </div>
          ...
          <div className="max-w-7xl mx-auto px-6">
            <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
              <div className="max-w-2xl">
                <h2 className="text-4xl font-serif text-black mb-4">Your Workspace</h2>
                  ...
                  <div className="aspect-[4/3] overflow-hidden bg-zinc-100 relative">
                    ...
                    <div className="absolute top-3 left-3 bg-white/90 backdrop-blur-md px-2 py-1 rounded-md border border-zinc-200 shadow-sm">
                      <span className="text-[10px] font-bold uppercase tracking-wider text-zinc-800">
                        {index === 0 ? 'Latest Edit' : `Version ${history.length - index}`}
                      </span>
                    </div>
                  </div>
      ...
      <footer className="py-12 border-t border-zinc-200 bg-white">
         <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row justify-between items-center gap-6">
            <div className="flex items-center space-x-2">
               ...
               <span className="text-lg font-serif font-bold text-black">Roomify</span>
               <span className="text-zinc-400 text-xs ml-2 font-mono">© 2024 INC.</span>
            </div>
...
const LogoPlaceholder = ({ text, icon }: { text: string, icon: React.ReactNode }) => (
   <div className="flex items-center justify-center space-x-2 group cursor-pointer">
      <div className="text-zinc-800 group-hover:text-black transition-colors">{icon}</div>
      <span className="font-bold text-xl text-zinc-800 group-hover:text-black transition-colors tracking-tight">{text}</span>
   </div>
);
...
const FooterLink = ({ href, label }: { href: string, label: string }) => (
  <a href={href} className="text-zinc-500 hover:text-black transition-colors text-xs font-bold uppercase tracking-wide">
    {label}
  </a>
);
